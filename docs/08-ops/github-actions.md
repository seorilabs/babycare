...
- 현재 `seorilabs/babycare`는 public repo다. caller는 public/private 양쪽을 고려해 `github.event.repository.private` 조건을 둔다.
...
private 저장소와 중앙 워크플로를 고칠 때의 참고값이다.

public 저장소의 GitHub-hosted 표준 러너는 분당 과금이 없다. public 전환의 목적이 조직 Actions
쿼타 보호였고, 이 라우팅이 그 목적을 달성하는 경로다.

## Central Source

수정 전 확인한다. `$SEORI_WORKSPACE`는 운영자 로컬의 Seorilabs 워크스페이스 루트다.

```bash
cat "$SEORI_WORKSPACE"/kubectl/github-actions-runners/global-versions.yaml
```
