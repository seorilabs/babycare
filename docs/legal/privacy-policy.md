...
> **상태: 초안 (`확정 필요`).** 이 문서는 제품 특성에 맞춰 작성한 초안이며, 법률 검토와 실제 데이터 처리 구성 확정 후 공개 URL로 호스팅해야 한다. 호스팅 전에는 스토어 `privacyPolicyUrl`에 사용하지 않는다. 개정일·회사 법적 정보·문의 연락처는 게시 전 확정한다.
...
- 이용자는 앱 내에서 본인 기록의 삭제 및 계정 삭제를 요청할 수 있으며, 계정 삭제 시 연동된 개인정보와 로컬 캐시를 삭제합니다. *(계정 삭제·데이터 완전 삭제 절차는 릴리스 전 구현·검증 대상 — `확정 필요`.)*
