...
- 같은 날 후속 확인에서 GA4 BigQuery `analytics_549232169` dataset과 `events_20260813`~`events_20260816` daily table이 생성된 것을 readback했다. 이 export 구간은 이벤트 115건·사용자 7명·세션 5건이며, 신규 사용자 4명 중 온보딩 완료 2명(50%)·첫 기록 1명(25%)·D1 재방문 0명이다. 초대 생성은 2건·1명이고 `bc_invite_shared`·`bc_invite_joined`는 0건이다. 공개 `1.1.3`에는 공유 이벤트가 없고 표본도 작으므로 D1=0이나 공유=0을 제품 결론으로 확대하지 않는다.
...
- 남은 것: 현재 공개본 `1.1.3`에는 이후 main의 온보딩·첫 기록 가이드와 초대 설치 링크·`bc_invite_shared` 계측이 없다. 다음 고정 후보의 실기기 QA·deployment 승인과 App Store Marketing URL 반영을 완료해 공개한 뒤, `onboarding → first_log`, `invite_created → invite_shared → invite_joined`를 새 버전 cohort로 다시 본다. Platform의 `bc_invite_shared` allowlist도 source merge 뒤 regsync·운영 readback해야 한다.
...
- 검증: AIT 6 suites/14건, 전체 `pnpm run test:static`의 core 49건·mobile 328건·Functions 20건과 typecheck·lint·architecture·docs·store/workflow gate, `pnpm run check:ait`, `git diff --check`가 통과했다. `.ait`은 RN 0.84.0/0.72.6 양쪽 Android·iOS 번들 0 errors/0 warnings로 생성됐고 source map에서 새 달력 모듈, bundle에서 keyboard inset 처리를 확인했다.
- PR #54를 squash merge한 `main@21a8573`에서 `.ait`을 다시 만들고 SHA-256 `baf0e95078a7156a74c54b2c8b3bb50bfeaf233feb68b54049a7567d7973c495`, 3,153,626 bytes를 확인했다. `20260811-7` / `019ff016-36a2-75bc-a29e-ea987c0bfed4`로 비공개 업로드·test push했고 Console readback은 `CREATED`, `isTested=true`, `deployed=false`다. 실제 Toss 기기 날짜·키보드 재검증은 남아 있다.
