...
- `apps-in-toss/apps-in-toss.config.json`에 machine-readable monetization 값을 추가했다. 실제 AppsInToss Console 값은 로그인 세션 연결 뒤 readback하며, Terms·추가 feature URL과 Toss 앱 sandbox QA는 별도 게이트로 유지한다.
