...
- Home은 진행 중 수면을 bounded event 목록에서 추론하지 않고 explicit `activeSleep`을 받는다. 기본 local preview도 전체 snapshot을 `{events, activeSleep}` overview source로 원자 전달해 cloud feed와 같은 presentation 경계를 사용한다. Stats의 7일/30일 bucket은 device-local calendar의 `setDate` 경계를 사용하고 DST 변화에서도 명시적 `[from, to)` 범위를 core에 전달한다.
