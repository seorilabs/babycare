...
| platform custom token client 구현·live backend 대기 | Firebase Auth와 session/group/baby 생성 | native Firebase app은 platform bridge custom token으로 로그인하고 기존 anonymous ID token은 같은 uid로 전환한다. 신규 uid는 서버 생성이며 UID 불일치 시 fail closed 한다. direct anonymous는 `demo-babycare` Auth Emulator 전용. 남음: platform signer IAM·registry sync·배포, live UID 보존 smoke, App Check/rate limit, account recovery/deletion 정책 |
