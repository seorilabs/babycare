...
| 운영 callable 진입 복구·실기기 재확인 대기 | 초대 발급/수락 Functions와 client callable adapter | owner 초대 발급/공유와 6자리 코드 합류 UI, HMAC·만료·single-use·rate limit·audit 검증을 구현했다. 운영 `createInvite`/`acceptInvite`는 Secret Manager를 사용하며 Domain Restricted Sharing에 맞춰 Cloud Run Invoker IAM check를 비활성화했다. 무인 probe가 Firebase callable의 `UNAUTHENTICATED`까지 도달함을 확인했다. 남음: TestFlight 인증 owner 발급 재시도, 다른 계정·기기 수락, App Check 또는 edge rate limit |
