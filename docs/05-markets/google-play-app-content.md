...
| 개인 정보 | 기타 정보 | 앱 기능 | 선택 | **아기 이름·생년월일(선택)과 수유·기저귀·수면 돌봄 기록**. ⚠️ 아래 판단 참고 |
...
### ⚠️ 판단이 필요한 항목 (사용자 확정)
1. **돌봄 기록 분류**: 수유·기저귀·수면 기록을 Google 분류상 **"개인 정보 > 기타 정보"**로 신고 권장(비의료). "건강 및 피트니스 > 건강 정보"로 분류하면 더 엄격한 정책·표시가 적용되므로, **비의료 기록 도구** 포지셔닝과 일관되게 "기타 정보" 권장. 최종 판단은 사용자·legal 확인.
2. **계정 삭제 기능**: Data safety의 "삭제 요청 가능=예"는 실제 계정 삭제 경로 구현을 전제로 한다. 현재 익명 인증 기반이라 **계정 삭제 UX 구현·검증이 릴리스 전 필요**(product-spec blocker).
