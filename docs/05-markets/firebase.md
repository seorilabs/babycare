...
- Functions slice: `createInvite`·`acceptInvite`·`deleteAccount` production ACTIVE. AppsInToss GA4 중계 `logAnalyticsEvents`도 `GA4_MEASUREMENT_ID`와 Secret Manager의 `GA4_API_SECRET`을 사용해 production ACTIVE이며, 인증된 callable smoke에서 `accepted=1`을 확인했다(2026-08-09). AIT attestation용 `mintAitAppCheckToken`은 mTLS secret version 1과 revision `mintaitappchecktoken-00001-law`로 production ACTIVE다(2026-08-11).
- GA4 BigQuery export: link `wNHaEpdIQ3SwUe_4OuyUhA`는 2026-08-09에 생성됐지만 `firebase-measurement@system.gserviceaccount.com`의 BigQuery 권한 누락으로 초기 dataset 생성이 지연됐다. 2026-08-18 `roles/bigquery.user` 부여·readback 뒤 비용이 발생하는 streaming export는 끄고 daily export만 유지했다. 2026-08-21 `analytics_549232169` dataset과 `events_20260813`~`events_20260816` daily table을 BigQuery API로 readback해 복구를 확인했다.
...
| Cloud Functions / Run | 예 | 초대·삭제 callable 운영. `logAnalyticsEvents`는 인증·allowlist·PII key 차단 후 GA4 Measurement Protocol로 최대 20개를 중계하며 production ACTIVE. `mintAitAppCheckToken`은 Toss mTLS login 검증 후 AIT Web app ID의 1시간 App Check token을 발급하며 production ACTIVE |
| Analytics | 예 | GA4 property `549232169`, Android/iOS/Web data stream 운영. BigQuery는 daily export만 사용하고 streaming export는 2026-08-18에 비활성화했다. Realtime `core_screen_view=1` 및 callable `accepted=1` readback. 동일 제품 이벤트를 Platform Events에도 fan-out |
...
| App Check | 예 | mobile에 Play Integrity·App Attest·DeviceCheck fallback 적용. production callable `ENFORCE_APP_CHECK=true`, Platform `require_app_check=true`. AIT는 `appLogin`·mTLS custom provider, 운영 secret·Function과 새 비공개 후보 연결 완료. 유효 Toss code의 token 발급·갱신 실기기 QA pending |
...
| `AIT_LOGIN_CLIENT_CERT` | `defineSecret` | AppsInToss login API mTLS certificate, production version 1 연결 완료 |
| `AIT_LOGIN_CLIENT_KEY` | `defineSecret` | AppsInToss login API mTLS private key, production version 1 연결 완료 |
...
- AIT mTLS secret과 mint Function은 production 연결됐다. `20260811-6` 비공개 번들을 로그인된 실제 Toss 설치 기기에서 열어 유효 code 기반 token 발급·갱신·복구 절차를 검증한다.
