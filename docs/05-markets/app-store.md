...
- **`v1.1.10` GitHub Actions 첫 후보 (2026-09-19)** — source `0bc9951062eaff2443584c7dbdea1c09b3fd2abe`, GitHub Actions run `35424137734`가 `macos-26`/Xcode 26.6에서 archive·업로드에 성공했다. `MARKETING_VERSION=1.1.10`, `CURRENT_PROJECT_VERSION=1001010`으로 태그에서 파생했고 archive Info.plist readback이 태그 값과 일치했다. `exportArchive`가 `Upload succeeded. / Uploaded BabyCare`를 출력했다. **App Review 제출은 하지 않았다.** ASC build id·`processingState`·`internalBuildState`·테스터 수는 아직 readback하지 않아 `app-store.config.json`에 `확정 필요 — ASC readback`으로 남겼다. RN prebuilt framework(React, ReactNativeDependencies, hermesvm) dSYM 누락으로 심볼 업로드는 경고와 함께 실패했다(이슈 #139).
