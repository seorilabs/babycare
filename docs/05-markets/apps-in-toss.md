...
- Onboarding input fix: 문자열 생년월일 입력을 오늘 이후 날짜를 막는 AIT 호환 달력으로 교체하고, 온보딩·체온·복약 입력에 keyboard inset·포커스 자동 스크롤을 적용했다. local `.ait` Android/iOS 양쪽 빌드는 통과했지만 이 수정본의 Console 업로드·실제 Toss 기기 재검증은 아직 남아 있다
...
- Release review prerequisite: 기존 후보 test push·`isTested=true`와 실제 Toss 로그인 기반 App Check token 발급·그룹·membership·아기 생성은 확인했다. 새 날짜·키보드 수정 후보를 다시 test push한 뒤 Storage 재실행 복구·토큰 만료 갱신·초대·공동 기록·네트워크 복귀를 확인하고 검수를 요청한다
