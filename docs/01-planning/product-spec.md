...
| AppsInToss | Granite RN·TDS UI, `Storage` session, Platform custom-token/Firebase Auth REST, Firestore REST 기록·조회, callable 초대·삭제 adapter를 연결했다. Toss Login·mTLS App Check mint 경계를 production에 연결하고 `main@2704ff7` 후보 `20260811-6` / `019fee32-8415-761c-be38-9c5769aa00b2`를 비공개 업로드·test push했다. | 로그인된 실제 Toss 설치 기기에서 App Check token·Storage·초대·기록·재실행·네트워크 복귀 QA, 운영 광고 검증 |
| release | 3마켓 식별자·자산·메타데이터 원장을 구성했고 체온·복약 `v1.1.3`을 Play internal `completed`·TestFlight `IN_BETA_TESTING`, AppsInToss 비공개 후보까지 업로드했다. Google Play·App Store 정책·개인정보 입력과 EU 포함 전국가 availability·DSA trader·비규제 의료기기 선언을 readback했다. | `v1.1.3` 실제 계정·기기 QA, App Check·광고 확인, 심사·공개 readback |
