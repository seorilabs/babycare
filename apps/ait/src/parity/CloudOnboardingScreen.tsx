...
export function CloudOnboardingScreen(props: {
  ...
  const next = async () => {
    if (submissionInFlight.current || saving) return;
    ...
    if ((step === 'birthDate' && isSelectableBirthDate(birthDate, new Date())) || (step === 'inviteCode' && codeValid)) {
  ...
  const back = () => {
    ...
    else if (step === 'birthDate') setStep('babyName');
  };
  const actionEnabled = useMemo(() => {
    if (step === 'caregiver') return caregiverValid;
    if (step === 'babyName') return babyNameValid;
    if (step === 'birthDate') return isSelectableBirthDate(birthDate, new Date());
    ...
    <SafeAreaView testID="cloud-onboarding-safe-area" style={[styles.fill, {backgroundColor: props.theme.colors.background}]}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.fill}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={[styles.mark, {backgroundColor: props.theme.colors.primarySoft}]}><Text style={styles.markEmoji}>🌿</Text></View>
          <Text style={[styles.eyebrow, {color: props.theme.colors.primary}]}>{strings.onboarding.eyebrow}</Text>
          {step === 'choose' ? (
            <>
              <Text style={[styles.title, {color: props.theme.colors.text}]}>{strings.onboarding.chooseTitle}</Text>
              <Text style={[styles.subtitle, {color: props.theme.colors.textMuted}]}>{strings.onboarding.chooseSubtitle}</Text>
              <View testID="onboarding-benefits" style={[styles.benefits, {backgroundColor: props.theme.colors.surface, borderColor: props.theme.colors.border}]}>
                {[{icon: '⚡️', title: strings.onboarding.quickBenefitTitle}, {icon: '🤝', title: strings.onboarding.handoffBenefitTitle}, {icon: '🔒', title: strings.onboarding.privateBenefitTitle}].map(benefit => (
                  <View key={benefit.title} style={styles.benefitItem}><Text style={styles.benefitIcon}>{benefit.icon}</Text><Text style={[styles.benefitTitle, {color: props.theme.colors.text}]}>{benefit.title}</Text></View>
                ))}
              </View>
              <View style={styles.choiceList}>
                {[{mode: 'create' as const, title: strings.onboarding.createChoiceTitle, description: strings.onboarding.createChoiceDescription}, {mode: 'join' as const, title: strings.onboarding.joinChoiceTitle, description: strings.onboarding.joinChoiceDescription}].map(choice => (
                  <Pressable accessibilityLabel={choice.title} accessibilityRole="button" key={choice.mode} onPress={() => {setMode(choice.mode); setStep('caregiver'); setErrorMessage(undefined);}} style={[styles.choice, {backgroundColor: props.theme.colors.surface, borderColor: props.theme.colors.border}]}>
                    <Text style={[styles.choiceTitle, {color: props.theme.colors.text}]}>{choice.title}</Text><Text style={[styles.choiceDescription, {color: props.theme.colors.textMuted}]}>{choice.description}</Text>
                  </Pressable>
                ))}
              </View>
            </>
            ...
            <>
              <Text style={[styles.progress, {color: props.theme.colors.textMuted}]}>{currentStep} / {totalSteps}</Text>
              <Text style={[styles.title, {color: props.theme.colors.text}]}>{step === 'caregiver' ? strings.onboarding.caregiverTitle : step === 'babyName' ? strings.onboarding.babyNameTitle : step === 'birthDate' ? strings.onboarding.birthDateTitle : strings.onboarding.inviteCodeTitle}</Text>
              <Text style={[styles.subtitle, {color: props.theme.colors.textMuted}]}>{step === 'caregiver' ? strings.onboarding.caregiverSubtitle : step === 'babyName' ? strings.onboarding.babyNameSubtitle : step === 'birthDate' ? strings.onboarding.birthDateSubtitle : strings.onboarding.inviteCodeSubtitle}</Text>
          ...
          {errorMessage ? <Text accessibilityRole="alert" style={[styles.error, {color: props.theme.colors.danger}]}>{errorMessage}</Text> : null}
          <Text style={[styles.notice, {color: props.theme.colors.textMuted}]}>{strings.onboarding.notice}</Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  fill: {flex: 1}, content: {flexGrow: 1, paddingBottom: 36, paddingHorizontal: 24, paddingTop: 38}, mark: {alignItems: 'center', borderRadius: 24, height: 68, justifyContent: 'center', marginBottom: 24, width: 68}, markEmoji: {fontSize: 34}, eyebrow: {fontSize: 11, fontWeight: '800', letterSpacing: 1.5}, progress: {fontSize: 13, fontWeight: '800', marginTop: 30}, title: {fontSize: 32, fontWeight: '800', letterSpacing: -1.2, marginTop: 8}, subtitle: {fontSize: 15, lineHeight: 23, marginTop: 12}, benefits: {alignItems: 'stretch', borderRadius: 18, borderWidth: 1, flexDirection: 'row', gap: 6, marginTop: 20, paddingHorizontal: 8, paddingVertical: 13}, benefitItem: {alignItems: 'center', flex: 1, justifyContent: 'flex-start'}, benefitIcon: {fontSize: 20}, benefitTitle: {fontSize: 11, fontWeight: '800', lineHeight: 15, marginTop: 5, textAlign: 'center'}, choiceList: {gap: 10, marginTop: 16}, choice: {borderRadius: 18, borderWidth: 1, padding: 20}, choiceTitle: {fontSize: 17, fontWeight: '800'}, choiceDescription: {fontSize: 13, lineHeight: 20, marginTop: 6}, fieldArea: {marginTop: 38}, input: {borderRadius: 14, borderWidth: 1, fontSize: 16, minHeight: 56, paddingHorizontal: 16}, codeInput: {fontSize: 22, fontWeight: '900', letterSpacing: 5, textAlign: 'center'}, actions: {flexDirection: 'row', gap: 10, marginTop: 24}, backButton: {alignItems: 'center', borderRadius: 16, borderWidth: 1, justifyContent: 'center', minHeight: 56, width: 88}, backText: {fontSize: 16, fontWeight: '800'}, nextButton: {alignItems: 'center', borderRadius: 16, flex: 1, justifyContent: 'center', minHeight: 56}, buttonText: {color: '#FFFFFF', fontSize: 16, fontWeight: '800'}, error: {fontSize: 12, fontWeight: '700', lineHeight: 18, marginTop: 11, textAlign: 'center'}, notice: {fontSize: 11, lineHeight: 17, marginTop: 'auto', paddingTop: 20, textAlign: 'center'},
});
