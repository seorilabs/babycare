...
import {checkMobileUpdateGate} from '../src/app/platform-update-gate';
...
test('recommended면 상태를 돌려주고 노출 이력을 남긴다', async () => {
  ...
  await expect(checkMobileUpdateGate()).resolves.toEqual(state);
