...
native Firebase app이 등록된 빌드는 Emulator로 fallback하지 않고 cloud mode를 사용한다. production native client config와 platform custom-token Auth bridge는 활성화됐고 Functions region은 `asia-northeast3`다. App Check 또는 edge rate limit과 실제 기기 2대 초대·동기화 QA는 남아 있다.
...
Firebase callable SDK가 Auth token을 포함한 요청을 함수까지 전달할 수 있도록 `createinvite`와 `acceptinvite` Cloud Run service는 Invoker IAM check를 비활성화한다. 조직의 Domain Restricted Sharing 정책에서 `allUsers` binding을 대신하는 계약이며 Firebase callable의 Auth 검증과 함수의 owner/membership 검사는 그대로 유지한다.
