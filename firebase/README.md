...
- `functions/src/`: `createInvite`, `acceptInvite`, `deleteAccount`, `logAnalyticsEvents`, `mintAitAppCheckToken`, HMAC/rate-limit/transaction·계정 삭제·GA4 relay·AIT mTLS attestation 구현
...
native Firebase app이 등록된 빌드는 Emulator로 fallback하지 않고 cloud mode를 사용한다. production native client config와 platform custom-token Auth bridge는 활성화됐고 Functions region은 `asia-northeast3`다. Android Play app signing SHA-256과 Play Integrity API, iOS Team ID·App Store ID와 App Attest·DeviceCheck provider 설정을 운영에서 readback했다. mobile App Check와 Platform 검증 경계는 `ENFORCE_APP_CHECK=true`, Platform `require_app_check=true`로 운영 중이다. sideload QA용 debug SHA와 unrecognized-version 허용은 제거했다. AIT custom provider의 mTLS secret version 1과 `mintAitAppCheckToken` production Function은 연결됐고, 로그인된 실제 Toss 설치 기기의 token 발급·갱신 QA가 남아 있다.
...
- `ENFORCE_APP_CHECK`: 기본 false이지만 production Parameter는 true
...
- `AIT_LOGIN_CLIENT_CERT`: AppsInToss login API mTLS client certificate, Secret Manager 연결·값 repo/client 금지
- `AIT_LOGIN_CLIENT_KEY`: AppsInToss login API mTLS private key, Secret Manager 연결·값 repo/client 금지

## Production callable 진입 계약

Firebase callable SDK와 AIT attestation HTTPS 요청이 함수까지 도달할 수 있도록 `createinvite`, `acceptinvite`, `deleteaccount`, `loganalyticsevents`, `mintaitappchecktoken` Cloud Run service는 Invoker IAM check를 비활성화한다. 조직의 Domain Restricted Sharing 정책에서 `allUsers` binding을 대신하는 계약이다. callable은 Firebase Auth/App Check와 함수 권한 검사를 유지하고, mint endpoint는 Toss mTLS 로그인 교환 성공 뒤에만 App Check token을 발급한다. App Check token 서명을 위해 Functions runtime 서비스 계정에는 자기 자신에 한정한 `roles/iam.serviceAccountTokenCreator`를 부여한다.
...
로컬 운영에서는 등록된 GCP session wrapper를 사용하도록 `SEORILABS_GCLOUD_BIN`에
`~/.config/seorilabs/scripts/gcloud-cli.sh`를 지정한다. CI는 기존 `gcloud`를 사용한다.
